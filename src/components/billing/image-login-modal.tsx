...
export function ImageLoginModal() {
  ...
  const dismissLoginModal = useImageQuotaStore(
    (state) => state.dismissLoginModal,
  );
    ...
    <div className="fixed inset-0 z-[320] flex items-center justify-center bg-black/45 p-4">
      <div className="relative w-full max-w-md rounded-3xl bg-white p-6 shadow-xl dark:bg-zinc-900">
        ...
        <h2 className="text-2xl font-semibold text-foreground">
          Ücretsiz kotanız bitti
        </h2>
        <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
          Misafir kotanız doldu (ör. {GUEST_IMAGE_LIMIT} görsel). Giriş
          yaparsanız ücretsiz aylık haklar tanınır — ekstra ücret yok.
        </p>
